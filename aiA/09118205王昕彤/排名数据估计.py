...
print(college_data[31:44])
college_batch_data = college_data[31:44] # 取15条数据
...
result = open("09118205王昕彤.csv","w",encoding="utf-8-sig",newline='')
                    ...
                    if os.path.exists(temp):
                        with open(temp,'r',encoding='UTF-8') as load_f:
                            ...
                            if (str(low_score) in load_dict[str(y)][p][c]) & (str(high_score) in load_dict[str(y)][p][c]):
                                ...
                                number = (low_rank-high_rank) // 25 + 1 #至少一名，整除18代表近似该分数段有30/88的学生去该大学
